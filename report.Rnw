...
% global settings
<<GlobalSettings, echo=FALSE>>=
library(knitr)
library(latex2exp)
library(ggplot2)
library(vctrs)
library(xtable) # package for tables in LaTeX format
opts_chunk$set(fig.path='figure/', fig.align='center', fig.pos='H',fig.width=4, fig.height=3)
# Remark: each settings can be individually set in each chunk 
@
...
<<SummaryTable, echo=TRUE, eval=TRUE, results='asis'>>=
...
\begin{figure}[H]
<<IrisPlot, fig.width=8, fig.height=6, echo=TRUE>>=
...
We can display the values of the variables computed in R chunks. For example, the average length of the petals in Iris data is \Sexpr{pl_mean}. We can also performs simple computations inside \textit{Sexpr}, for example: \Sexpr{round(mean(iris_data$Petal.Length),3)}.
